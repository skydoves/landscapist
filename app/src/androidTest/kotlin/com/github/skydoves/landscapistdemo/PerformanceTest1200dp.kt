...
/**
 * Performance comparison with 1200.dp image size.
 * Run each test manually to collect 5 rounds of data.
 *
 * Run with:
 * ./gradlew :app:connectedDebugAndroidTest \
 *   -Pandroid.testInstrumentationRunnerArguments.class=com.github.skydoves.landscapistdemo.PerformanceTest1200dp
 */
@LargeTest
@RunWith(AndroidJUnit4::class)
class PerformanceTest1200dp {
  ...
  fun glideImagePerformance() {
    println("\n[GlideImage] Performance Test (1200.dp)...")
    ...
    val loadTime = measureTimeMillis {
      composeTestRule.setContent {
        GlideImage(
          ...
          modifier = Modifier.size(1200.dp).testTag("GlideImage"),
  ...
  fun coilImagePerformance() {
    println("\n[CoilImage] Performance Test (1200.dp)...")
    ...
    val loadTime = measureTimeMillis {
      composeTestRule.setContent {
        CoilImage(
          imageModel = { TEST_IMAGE },
          modifier = Modifier.size(1200.dp).testTag("CoilImage"),
          imageOptions = ImageOptions(),
        )
      }
  ...
  fun landscapistImagePerformance() {
    println("\n[LandscapistImage] Performance Test (1200.dp)...")
    ...
    val loadTime = measureTimeMillis {
      composeTestRule.setContent {
        LandscapistImage(
          imageModel = { TEST_IMAGE },
          modifier = Modifier.size(1200.dp).testTag("LandscapistImage"),
          imageOptions = ImageOptions(),
  ...
  fun frescoImagePerformance() {
    println("\n[FrescoImage] Performance Test (1200.dp)...")
    ...
    val loadTime = measureTimeMillis {
      composeTestRule.setContent {
        FrescoImage(
          imageUrl = TEST_IMAGE,
          modifier = Modifier.size(1200.dp).testTag("FrescoImage"),
          imageOptions = ImageOptions(),
        )
      }
