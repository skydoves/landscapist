...
* A sub-sampled image measured to nothing on an axis froze the thread it was laid out on. The tile grid halved a sample size until it no longer fit, and a viewport of zero divides to infinity: the counter climbed to 2^30, overflowed, reached zero and stayed there with the loop still running. Both ends are bounded now.
* A composable that measured to nothing on its first pass, which is a lazy item laid out before its container has room or a container that animates open from nothing, locked that zero as its answer. A zero reads back as an unbounded axis, so both axes zero left the request with no target at all: a decode at the source's own size, and a cache read with no box to judge a variant against. It waits for a measurement worth locking, which is what the single node path always did.
