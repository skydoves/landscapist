...
/**
 * Fetcher for [DrawableResModel] image models (drawable resource IDs).
 *
 * Loads the drawable from Android resources and returns it as a pre-decoded result.
 *
 * Example usage:
 * ```kotlin
 * LandscapistImage(
 ...
 * ```
 */
public class DrawableResFetcher : ImageFetcher {

  override fun canHandle(model: Any?): Boolean {
    return model is DrawableResModel
  }

  override suspend fun fetch(request: ImageRequest): FetchResult {
    val resModel = request.model as? DrawableResModel
      ?: return FetchResult.Error(IllegalArgumentException("Model is not a DrawableResModel"))

    return withContext(Dispatchers.IO) {
      try {
        val context = AndroidContextProvider.get()
        val drawable = ContextCompat.getDrawable(context, resModel.resId)
          ?: return@withContext FetchResult.Error(
            Resources.NotFoundException("Drawable resource not found: ${resModel.resId}"),
          )
