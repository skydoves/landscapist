...
internal fun decodeComparison() {
  ...
  println("    coil (skia)    ${coilBytes.formatBytes()}")
  println(
    "    Not a like for like number. Skia decodes into native memory, which this cannot see, so " +
      "Coil's row is only its JVM side. Landscapist's is the whole thing: the raster it no " +
      "longer materialises was 48 MB of Java heap.",
  )
  println()
}
