...
internal fun peakMemoryComparison() {
  ...
  println(
    "peak memory decoding a ${PHOTO_WIDTH}x$PHOTO_HEIGHT JPEG " +
      "(${photo.size.toLong().formatBytes()}) down to ${TARGET_WIDTH}x$TARGET_HEIGHT",
  )

  val decoder = createPlatformDecoder()
  val config = LandscapistConfig()
  val paths = listOf(
    DecodePath(LANDSCAPIST_PATH, "landscapist") {
      runBlocking {
    ...
    DecodePath(FULL_RASTER_PATH, "no-subsampling") { fullDecodeThenScale(photo) },
    // Coil's own decoder rather than a snippet that resembles it.
    DecodePath(COIL_PATH, "coil") { coilDecode(photo) },
  )

  // Warm every path first, or a cold reader registry and a cold codec table land on whichever
  // path runs first.
  for (path in paths) repeat(3) { path.decode() }

  println("  peak Java heap above the resting heap, median of 5")
  for (path in paths) {
    val median = LongArray(5) { peakHeapBytes { path.decode() } }.median()
    Metrics.record("peak-heap.${path.key}.bytes", median)
    println("    ${path.name.padEnd(28)}${median.formatBytes()}")
  }
  ...
  for (path in paths) {
    val median = LongArray(3) { childPeakResident(path.name, photoFile.absolutePath) }.median()
    Metrics.record("peak-resident.${path.key}.bytes", median)
    println("    ${path.name.padEnd(28)}${median.formatBytes()}")
  }
  println(
...
/** One decode path, its table label and the key its numbers are recorded under. */
private class DecodePath(val name: String, val key: String, val decode: () -> Unit)

private const val LANDSCAPIST_PATH = "landscapist"
private const val FULL_RASTER_PATH = "same stack, no subsampling"
private const val COIL_PATH = "coil (skia decoder)"
...
internal fun runDecodeChild(path: String, photoPath: String) {
  ...
  coilDecode(thumbnail)
  System.gc()
  println("ready")
  System.out.flush()
  readLine()
  when (path) {
    LANDSCAPIST_PATH -> runBlocking {
      val result = decoder.decode(photo, "image/jpeg", TARGET_WIDTH, TARGET_HEIGHT, config)
      check(result is DecodeResult.Success) { "decode failed: $result" }
    }
    FULL_RASTER_PATH -> fullDecodeThenScale(photo)
    else -> coilDecode(photo)
  }
  println("done")
  System.out.flush()
}
...
internal fun fullDecodeThenScale(bytes: ByteArray) {
  val full = ImageIO.read(java.io.ByteArrayInputStream(bytes)) ?: error("decode failed")
  val scaled = BufferedImage(TARGET_WIDTH, TARGET_HEIGHT, BufferedImage.TYPE_INT_ARGB)
  val g = scaled.createGraphics()
  try {
    g.drawImage(full, 0, 0, TARGET_WIDTH, TARGET_HEIGHT, null)
  } finally {
    g.dispose()
  }
  full.flush()
